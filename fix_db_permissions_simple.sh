#!/bin/bash
# Простой скрипт для исправления прав доступа к базе данных SQLite

set -e

# Определяем путь к проекту
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DB_PATH="${PROJECT_DIR}/telespam.db"
DB_DIR="$(dirname "$DB_PATH")"

echo "=========================================="
echo "Исправление прав доступа к базе данных"
echo "=========================================="
echo ""
echo "📁 Путь к базе данных: $DB_PATH"
echo "📂 Директория: $DB_DIR"
echo ""

# Создаем директорию если не существует
if [ ! -d "$DB_DIR" ]; then
    echo "📁 Создаю директорию: $DB_DIR"
    mkdir -p "$DB_DIR"
fi

# Создаем файл БД если не существует
if [ ! -f "$DB_PATH" ]; then
    echo "📄 Создаю файл базы данных: $DB_PATH"
    touch "$DB_PATH"
fi

# Показываем текущие права
echo "Текущие права:"
ls -la "$DB_PATH" 2>/dev/null || echo "⚠️  Файл не существует"
echo ""

# Исправляем права на директорию
echo "🔧 Исправляю права на директорию..."
chmod 755 "$DB_DIR" 2>/dev/null || {
    echo "⚠️  Не удалось установить права на директорию (может потребоваться sudo)"
    echo "   Попробуйте: sudo chmod 755 $DB_DIR"
}

# Исправляем права на файл БД
echo "🔧 Исправляю права на файл базы данных..."
chmod 664 "$DB_PATH" 2>/dev/null || {
    echo "⚠️  Не удалось установить права на файл (может потребоваться sudo)"
    echo "   Попробуйте: sudo chmod 664 $DB_PATH"
}

# Если запущено от root или с sudo, также исправляем владельца
if [ "$EUID" -eq 0 ] || [ -n "$SUDO_USER" ]; then
    # Определяем пользователя gunicorn или текущего пользователя
    GUNICORN_USER=$(systemctl show -p User telespam-web.service 2>/dev/null | cut -d= -f2 || echo "")
    
    if [ -z "$GUNICORN_USER" ]; then
        GUNICORN_USER=$(ps aux | grep -E 'gunicorn|flask' | grep -v grep | head -1 | awk '{print $1}' || echo "")
    fi
    
    if [ -z "$GUNICORN_USER" ] && [ -n "$SUDO_USER" ]; then
        GUNICORN_USER="$SUDO_USER"
    fi
    
    if [ -n "$GUNICORN_USER" ]; then
        echo "👤 Устанавливаю владельца: $GUNICORN_USER"
        chown "$GUNICORN_USER:$GUNICORN_USER" "$DB_PATH" 2>/dev/null || echo "⚠️  Не удалось изменить владельца"
        chown "$GUNICORN_USER:$GUNICORN_USER" "$DB_DIR" 2>/dev/null || echo "⚠️  Не удалось изменить владельца директории"
    fi
fi

# Проверяем результат
echo ""
echo "✅ Новые права:"
ls -la "$DB_PATH"
echo ""

# Проверяем доступность
if [ -r "$DB_PATH" ] && [ -w "$DB_PATH" ]; then
    echo "✅ База данных доступна для чтения и записи"
else
    echo "⚠️  База данных может быть недоступна для записи"
    echo "   Попробуйте запустить с sudo: sudo bash $0"
fi

echo ""
echo "✅ Готово!"
echo ""
echo "Если проблема сохраняется, проверьте:"
echo "   1. Права на файл: ls -la $DB_PATH"
echo "   2. Права на директорию: ls -la $DB_DIR"
echo "   3. Владелец файла должен совпадать с пользователем веб-сервера"
echo ""

